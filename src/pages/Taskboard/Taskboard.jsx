...
import { getTasksByStory, updateTask } from "../../services/taskService";
...
export default function Taskboard() {
  ...
  const handleSaveEstimatedHours = async (taskId, value) => {
    ...
    try {
      ...
      setTasks((prev) =>
        prev.map((t) =>
          t.id === taskId ? { ...t, estimated_hours: parsed } : t
        )
      );
    } catch (err) {
        ...
        prev.map((t) =>
          t.id === taskId
            ? {
                ...t,
                assignee: userId,
                assignee_username: assignedMember?.user.username,
              }
            : t
        )
      );
                ...
                <div className="story-cell">
                  ...
                  <div className="story-main">
                    <div
                      ...
                      title={story.title}
                    >
                      <span className="story-ref">#{story.id}</span>{" "}
                      {story.title}
                    </div>
                  </div>
                  ...
                  {!isCollapsed && (
                    <div className="task-cell">
                      {tasksForCell(story.id, col.key).map((task) => {
                        ...
                        // per-task: assignee or dev or superuser can edit actual hours
                        ...
                        return (
                          <div key={task.id} className="task-card">
                            ...
                            {/* ── ESTIMATED HOURS ── */}
                            <div className="task-meta">
                              {editingEstimatedHours?.id === task.id ? (
                                <input
                                  className="task-hours-input"
                                  type="number"
                                  min="0"
                                  step="0.5"
                                  value={editingEstimatedHours.value}
                                  autoFocus
                                  onChange={(e) =>
                            ...
                            {/* <div className="task-meta">
                              {editingActualHours?.id === task.id ? (
                                <input
                                  ...
                                  min="0"
                                  step="0.5"
                                  value={editingActualHours.value}
                                  autoFocus
                                  onChange={(e) =>
                ...
                {members.map((membership) => (
                  <div key={membership.id} className="member-item">
                    ...
                    <button
                      className="member-item__assign-btn"
                      onClick={() =>
                        handleAssignTask(assigningTaskId, membership.user.id)
                      }
                      disabled={assigningUsers.has(membership.user.id)}
                      title="Assign to this member"
                    >
